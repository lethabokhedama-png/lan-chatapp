...
Ensures DATA/dev/flags.json has all current flags with defaults.
"""
import json
...
def ensure_flags(data_path: str):
    ...
    # Merge — keep existing values, add any missing defaults
...
def get_flags(data_path: str, uid: int = None) -> dict:
    ...
    try:
        ...
        if uid:
            user_overrides = data.get("users", {}).get(str(uid), {})
